      ...
      <div className="grid gap-6 lg:grid-cols-12">
        <Card padding className="p-3 lg:col-span-3 xl:col-span-2">
          <nav className="space-y-1 max-h-[70vh] overflow-y-auto">
            {SCHOOL_SETTINGS_SECTIONS.map((s) => (
              <Link
                key={s.key}
                to={`/school-settings/${s.key}`}
                className={cn(
        ...
        <Card className="lg:col-span-9 xl:col-span-10">
          <h3 className="text-lg font-semibold mb-4">{currentSection.label}</h3>
          ...
          {section === 'number_formats' && (
            <div className="mb-4 flex items-center gap-2">
              ...
              {preview && <span className="font-mono text-sm text-primary">{preview}</span>}
            </div>
          )}
