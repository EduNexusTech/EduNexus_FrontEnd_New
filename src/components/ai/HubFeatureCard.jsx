...
export default function HubFeatureCard({ to, icon: Icon, title, description, accent = 'primary' }) {
  return (
    <Link to={to} className="block h-full">
      <Card hover className={cn('clay-stat-3d flex h-full flex-col', accents[accent] || accents.primary)}>
        ...
        <p className="mt-2 flex-1 text-sm text-[var(--clay-primary-soft)]">{description}</p>
